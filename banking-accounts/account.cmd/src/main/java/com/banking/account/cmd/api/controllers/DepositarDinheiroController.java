...
public class DepositarDinheiroController {
    private final Logger logger = Logger.getLogger(DepositarDinheiroController.class.getName());
    ...
    public ResponseEntity<BaseResponse> depositFunds(@PathVariable(value="id") String id,
                                                     @RequestBody DepositarDinheiroCommand command){
        try{
            command.setId(id);
